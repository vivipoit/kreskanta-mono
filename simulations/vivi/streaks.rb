...
{ # rubocop:disable Metrics/BlockLength
  ...
  prices.each_with_index do |price, index|
    ...
    if price.price > previous_price
      up_streak += 1
      down_streak = 0

      if up_streak > 5 && usd_balance > 100
        usd_balance -= 100

        btcs_bought = 100 / price.price
        btc_balance += btcs_bought
      end
    end

    next unless price.price < previous_price

    down_streak += 1
    up_streak = 0

    next unless down_streak > 5 && btc_balance.positive?

    btcs_sold = btc_balance / 4
  ...
  p "| streaks | #{name} | #{usd_balance.round(6)}"\
